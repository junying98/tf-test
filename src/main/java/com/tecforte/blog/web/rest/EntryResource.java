...
public class EntryResource {
    ...
    public ResponseEntity<EntryDTO> createEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
        ...
        if(blogDTO.isPresent()) {
        	...
        	String title = entryDTO.getTitle();
        	String content = entryDTO.getContent();
        	emojiContentCheck(isPositive,emoji,title,content);
        }
    ...
    public ResponseEntity<Void> deleteEntriesWithKeywords(String keywords) {
        log.debug("REST request to delete Blogs containing keywords : {}", keywords);
        List<String> keywordslist=new ArrayList<>(Arrays.asList(keywords.split(",")));
        List<EntryDTO> page = entryService.findAll(Pageable.unpaged()).getContent();
        List<Long> ids = new ArrayList<Long>();
        for(EntryDTO x:page) {
        	for(String y:keywordslist) {
        		String pattern=".*\\b"+y+"\\b.*";
    				if(x.getContent().matches(pattern)) {
    					ids.add(x.getId());
    					entryService.delete(x.getId());
    				}
        	}
        }
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, ids.toString())).build();
    }
    
    @DeleteMapping("/blogs/{id}/clean")
    public ResponseEntity<Void> deleteEntryWithKeywordsById(String keywords,@PathVariable Long id) {
        log.debug("REST request to delete Blogs containing keywords : {}", keywords);
        List<String> keywordslist=new ArrayList<>(Arrays.asList(keywords.split(",")));
        List<EntryDTO> page = entryService.findAll(Pageable.unpaged()).getContent();
        List<Long> ids = new ArrayList<Long>();
    ...
    public static void emojiContentCheck(boolean isPositive,Emoji emoji, String  entryTitle, String entryContent) {
    	Emoji[] positiveEmoji = {Emoji.LIKE,Emoji.HAHA};
    	Emoji[] negativeEmoji = {Emoji.ANGRY,Emoji.SAD};
    	String[] positiveKeywords = {"LOVE","HAPPY","TRUST"};
    	String[] negativeKeywords = {"SAD","FEAR","LONELY"};
    	entryTitle = entryTitle.toUpperCase();
    	entryContent = entryContent.toUpperCase();
    	
    	if(isPositive) {
    		for(int i=0;i<negativeEmoji.length;i++) {
    			if(negativeEmoji[i]==emoji) {
    				throw new BadRequestAlertException("Invalid Emoji", ENTITY_NAME, "invalidEmoji");
    			}
    		}
    		for(int i=0;i<negativeKeywords.length;i++) {
    			String pattern=".*\\b"+negativeKeywords[i]+"\\b.*";
				if(entryTitle.matches(pattern)) {
					throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
				}
				if(entryContent.matches(pattern)) {
					throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
				}
    		}
    	}
    		...
    		for(int i=0;i<positiveEmoji.length;i++) {
    			if(positiveEmoji[i]==emoji) {
    				throw new BadRequestAlertException("Invalid Emoji", ENTITY_NAME, "invalidEmoji");
    			}
    		}
    		for(int i=0;i<positiveKeywords.length;i++) {
    			String pattern=".*\\b"+positiveKeywords[i]+"\\b.*";
    				if(entryTitle.matches(pattern)) {
    					throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
    				}
    				if(entryContent.matches(pattern)) {
